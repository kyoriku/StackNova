...
//   async clearPostCache(postId, userId) {
//     try {
//       if (!client.isReady) return;
//       const keys = [
//         `post:${postId}`,
...
//       ];
//       if (userId) {
//         keys.push(`user:${userId}:posts`);
//       }
//       await Promise.all(keys.map(key => client.del(key)));
//       if (process.env.NODE_ENV === 'development') {
//         console.log('\x1b[36m%s\x1b[0m', `Cleared caches for keys:`, keys);
//       }
//     } catch (error) {
...
const redisService = {
  ...
  //     if (!client.isReady) return;
  //     await Promise.all([
  //       client.del(`post:${postId}`),
  //       client.del(`comments:${postId}`)
  //     ]);
