...
const seedComments = async (users, posts) => {
...

  {
    ...
    user_id: users[1].id, // Changed from users[3].id to users[1].id (avoid conflict with post 8 author)
    post_id: posts[8].id,
  ...
  {
    comment_text: "I've been using Playwright for about 6 months now and completely agree with your assessment. One tip that made our tests more reliable: use the built-in `expect` polling mechanism for async operations:\n\n```javascript\n// Instead of this (which might flake):\nawait page.click('.submit-button');\nexpect(await page.locator('.success-message')).toBeVisible();\n\n// Do this (which automatically retries the assertion):\nawait page.click('.submit-button');\nawait expect(page.locator('.success-message')).toBeVisible();\n```\n\nThis small change eliminated about 80% of our flaky tests overnight!",
    user_id: users[0].id,
    post_id: posts[2].id,
  ...
  
  {
    ...
    user_id: users[3].id, // Changed from users[4].id to users[3].id (avoid conflict with post 4 author)
    post_id: posts[4].id,
  ...
  
  {
    ...
    user_id: users[0].id, // Changed from users[2].id to users[0].id (avoid conflict with post 7 author)
    post_id: posts[7].id,
  ...
  {
    comment_text: "We had a similar challenge and switched to React Hook Form with Zod. The performance improvement was significant, especially for large forms. Here's a pattern that worked well for us:\n\n```jsx\nimport { useForm } from 'react-hook-form';\nimport { zodResolver } from '@hookform/resolvers/zod';\nimport { z } from 'zod';\n\n// Define schema with conditional validation\nconst formSchema = z.object({\n  accountType: z.enum(['personal', 'business']),\n  // Conditionally required fields\n  businessName: z.string().min(1, 'Required').optional()\n    .refine(val => {\n      return formData.accountType === 'business' ? !!val : true;\n    }, { message: 'Business name is required for business accounts' }),\n  // Other fields...\n});\n\nfunction SmartForm() {\n  // React Hook Form with Zod resolver\n  const { register, handleSubmit, watch, formState: { errors } } = useForm({\n    resolver: zodResolver(formSchema),\n    defaultValues: {\n      accountType: 'personal',\n      businessName: '',\n      // More defaults...\n    },\n  });\n  \n  // Watch values for conditional rendering/validation\n  const accountType = watch('accountType');\n  \n  return (\n    <form onSubmit={handleSubmit(onSubmit)}>\n      <div>\n        <label>Account Type</label>\n        <select {...register('accountType')}>\n          <option value=\"personal\">Personal</option>\n          <option value=\"business\">Business</option>\n        </select>\n      </div>\n      \n      {accountType === 'business' && (\n        <div>\n          <label>Business Name</label>\n          <input {...register('businessName')} />\n          {errors.businessName && <p>{errors.businessName.message}</p>}\n        </div>\n      )}\n      \n      <button type=\"submit\">Submit</button>\n    </form>\n  );\n}\n```\n\nFor multi-step forms, we use a parent form context with individual subforms for each step. The parent manages the overall state and validation, while each step handles its own fields.",
    user_id: users[3].id,
    post_id: posts[9].id,
  ...
  
  {
    ...
    user_id: users[4].id, // Changed from users[0].id to users[4].id (avoid conflict with post 0 author)
    post_id: posts[0].id,
  ...
  {
    comment_text: "For structuring tests, we found the Page Object Model works great with Playwright. Here's a simplified example:\n\n```javascript\n// pages/login.page.js\nclass LoginPage {\n  constructor(page) {\n    this.page = page;\n    this.usernameInput = page.locator('#username');\n    this.passwordInput = page.locator('#password');\n    this.loginButton = page.locator('button[type=\"submit\"]');\n  }\n\n  async navigate() {\n    await this.page.goto('/login');\n  }\n\n  async login(username, password) {\n    await this.usernameInput.fill(username);\n    await this.passwordInput.fill(password);\n    await this.loginButton.click();\n  }\n}\n\n// In your test:\nconst loginPage = new LoginPage(page);\nawait loginPage.navigate();\nawait loginPage.login('testuser', 'password');\n```\n\nThis keeps tests clean and makes maintenance much easier when your UI changes.",
    user_id: users[1].id,
    post_id: posts[2].id,
